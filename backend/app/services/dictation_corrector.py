...
async def correct_dictation(original: str, user_input: str) -> dict[str, Any]:
    ...
    为什么有重试：LLM 偶发超时或返回空时，重试 2 次可大幅减少触发兜底的概率，
    避免同一篇默写因网络抖动拿到差异悬殊的分数。退避间隔防止连续打爆 API。
    """
    user_prompt = f"【原文】\n{original}\n\n【学生输入】\n{user_input}"
    ...
    for attempt in range(3):
        try:
            ...
            if result and "score" in result:
                log.debug("LLM 批改成功，第 %d 次尝试", attempt + 1)
                break
            # 返回了但缺少 score 字段，视为无效
            log.debug("LLM 返回无效（第 %d 次），score 字段缺失，重试", attempt + 1)
        except Exception as e:  # noqa: BLE001
            log.debug("LLM 批改调用失败（第 %d 次）: %s，退避后重试", attempt + 1, e)
        # 最后一次不需要等待
        if attempt < 2:
            await asyncio.sleep(1.0 * (attempt + 1))

    if not result or "score" not in result:
        log.debug("LLM 3 次重试均失败，降级到本地兜底批改")
...
def _mock_correct(original: str, user_input: str) -> dict[str, Any]:
    ...
    # 校准评分：用线性映射让兜底分布与 LLM 区间对齐。
    # ratio 直接反映了错误率，不再叠加 error_count 惩罚（否则双重扣分过严）。
    # 公式：40 + ratio*60 → ratio=1.0→100, ratio=0.8→88, ratio=0.6→76, ratio=0.4→64
    total_errors = wrong_count + missing_count + extra_count
    if total_errors == 0:
        score = 100
    else:
        score = max(50, min(100, int(40 + ratio * 60)))
